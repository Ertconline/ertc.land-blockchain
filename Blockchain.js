 ...
 */
 ...
 function Blockchain(config) {
     ...
     function initP2PServer() {
        ...
        if(!config.program.leechMode) {
            ...
            if(config.sslMode) {
                ...
                wss = new WebSocket.Server({ server, maxPayload: config.maxPayload });
     ...
     function initConnection(ws) {
        const socketInfo = ws._socket;

        if (peersBlackList.indexOf(socketInfo.remoteAddress) !== -1) {
            if(config.program.verbose) {
                logger.info(`Blacklisted peer ${socketInfo.remoteAddress}`);
            }
        ...
        for (const socket of sockets) {
            if (
                socket._socket.remoteAddress === socketInfo.remoteAddress &&
                socket._socket.remotePort === socketInfo.remotePort &&
                socket.readyState === 1 &&
                !socket._isServer &&
                !config.program.ignoreDoubleConnections
            ) {
                if(config.program.verbose) {
                    logger.info(`Dublicated peer ${socketInfo.remoteAddress}`);
                }
            ...
            if (socket.readyState !== 1) {
                if(config.program.verbose) {
                    logger.info(`Connection Not ready. Peer ${socketInfo.remoteAddress}`);
                }
         ...
         ws.on('message', (data) => { 
             ...
             switch (message.type) {
                 ...
                 case MessageType.RESPONSE_BLOCKCHAIN:
                     handleBlockchainResponse(message, undefined, ws);
                     break;
     ...
     function sendAllBlockchain(ws, fromBlock, limit) {
        ...
        getAllChain(fromBlock, limit, function (blockchain) {
            // console.log('Gettings blocks for node, from:', fromBlock, 
            // 'limit:', limit, 'length', blockchain.length, 
            // 'index:', blockchain[0].index, '-', blockchain[blockchain.length - 1].index);
      ...
      * @param cb
      */
     function addBlock(newBlock, cb, noHandle=false) {
         getLatestBlock(function (lastestBlock) {
             if(isValidNewBlock(newBlock, lastestBlock)) {
                 addBlockToChain(newBlock, noHandle, cb);
             } else {
     ...
     function connectToPeers(newPeers) {
        ...
        for (const peer of newPeers) {
            ...
            try {
                ...
                const ws = new WebSocket(peer, { perMessageDeflate: false, maxPayload: config.maxPayload });
                ...
                ws.on('error', (e) => {
                    console.log('Socket close by error', e);
                    ws.close();
                });
            } catch (e) {
                if(config.program.verbose) {
      ...
      * @param message
      */
     function handleBlockchainResponse(message, preProcessedBlocks, ws) {
        ...
        getLatestBlock(function (latestBlockHeld) {
            ...
            try {
                if (latestBlockReceived.timestamp > moment().utc().valueOf() + (12 * 60 * 60 * 1000)) {
                    if (config.program.verbose) {
                ...
                if (latestBlockReceived.index > latestBlockHeld.index/* || 
                    (blockHandler.keyring.length === 0 && latestBlockReceived.index < 5 && latestBlockReceived.index !== 0)*/
                ) {
                    ...
                    if (latestBlockHeld.hash === latestBlockReceived.previousHash) { //когда получен один блок от того который у нас есть
                        if (isValidChain(receivedBlocks) && 
                            (firstBlockReceived.index <= maxBlock || receivedBlocks.length === 1)
                        ) {
                        ...
                        if (maxBlock === 0) {
                            if (!blockHandler.syncInProgress) {
                                broadcast(queryAllMsg(1, 2));
                            }
    
                            storj.put('chainResponseMutex', false);
                        } else {
                            let getBlockFrom = maxBlock - config.blockQualityCheck;
                            if (getBlockFrom < 0) {
                                getBlockFrom = 1;
                                lastKnownBlock = maxBlock;
                            }
                        ...
                        if (firstBlockReceived.index <= maxBlock + 1 && receivedBlocks.length > 1) {
                            replaceChain(receivedBlocks, function (err) {
                                getLatestBlock(function (latestBlockHeld) {
                                    if (!latestBlockHeld) {
                                        storj.put('chainResponseMutex', false);
                                        return;
                            ...
                            }, firstBlockReceived.index === 1 && latestBlockReceived.index === 3);
                        } else {
                            storj.put('chainResponseMutex', false);
                        }
                    }
      ...
      * @param {function} cb
      */
     function replaceChain(newBlocks, cb, enableResync=true) {
      ...
      * Запуск сети
      */
     async function startBlockchainServers() {
         connectToPeers(peers);
         initHttpServer();
         initP2PServer();
         createWalletIfNotExsists();
         
         if (config.program.keypoaEmission) {
            await keypoaEmission();
         }
     ...
     function getCurrentPeers(fullSockets) {
        return sockets.map(socket => {
            if (socket && socket.readyState === 1 && socketPasswordOk(socket)) {
                if (fullSockets) {
                    return socket;
                } else {
                    if (!socket._isServer) {
                        ...
                        return `ws://${socket._socket.remoteAddress}:${socket._socket.remotePort}`;
                    }
                }
            }
 ...
 //**************************************************************************
     ...
     async function keypoaEmission() {
        if(
            ...
            logger.info('Starting keypoa emission');

            const keypoa = storj.get('keypoa');
            const keystorage = keypoa.interface.getCurrentKeyStorage();
            keystorage[wallet.id] = { key: wallet.id, type: 'Admin' };
            keypoa.interface._replaceKeyStorage(keystorage);

            await keypoa.interface.issueKey(wallet.id, 'Admin')
            keypoa.interface.rewriteKeyFile(keystorage); 
            console.log('Keypoa: Keypoa accepted');

            console.log('Keypoa: Saving wallet file...');
            wallet.save();
            console.log('Keypoa: Saved wallet file!');
        } else {
            logger.error('Cant generate keypoa');
     ...
     async function start() {
        ...
        if(typeof config.ecmaContract !== 'undefined' && config.ecmaContract.enabled) {
            blockchainObject.ecmaContract = await EcmaContract.create();
     ...
     function getBlocksByids(ids, cb) {
        blockchain.getMany(ids, function (err, val) {
            if (err) {
                cb(err);
            } else {
                cb(null, val)
