...
const sleep = (time=2_000) => new Promise(resolve => setTimeout(resolve, time));
...
class EcmaContract {
    static async create() {
        const events = await EventsDB.create({ path: '/contractsRuntime/EventsDB.db' });
        const instance = new this(events);
        return instance;
    }
    ...
    constructor(events) {
        ...
        const blockHandlerQueueCheck = async () => {
            if (this.ecmaCalls.length === 0) return;
            if (this.ecmaCall) return;

            this.ecmaCall = true;
            const { blockData, block, callback } = this.ecmaCalls.shift();

            logger.info(`Handle block # ${block.index}`);

            this.events._handleBlockReplay(block.index, (err) => {
                if (err) {
                    callback(err);
                    return;
                }

                this._handleBlock(blockData, block, false, (err) => {
                    this.ecmaCall = false;
                    if (err) {
                        callback(err);
                        return;
                    ...
                    this._lastKnownBlock = block.index;
                    logger.info(`Done! # ${block.index}`);
                    setTimeout(async () => blockHandlerQueueCheck(), 1);
        ...
        const blockHandlerQueue = (blockData, block, callback) => {
            this.ecmaCalls.push({ blockData, block, callback });
            blockHandlerQueueCheck();
        };

        this.blockHandler.registerBlockHandler(EcmaContractDeployBlock.blockType, blockHandlerQueue);
        this.blockHandler.registerBlockHandler(EcmaContractCallBlock.blockType, blockHandlerQueue);
    ...
    createContractInstance(address, code, state, cb) {
        ...
        function createInstance(limits) {
            ...
            try {
                vm.setTimingLimits((limits.timeLimit + 10000) * multy);
                vm.setCpuLimit((limits.timeLimit + 500) * multy);
                ...
                vm.runContextMethodAsync('contract.init', async function (err) {
                    ...
                    if(typeof cb === 'function') {
                        cb({ vm, db, info: contractInfo, limits });
                    }
                });
    ...
    _setupVmFunctions(vm, db) {
            ...
            return {
                return: function (result) {
                    if(!result) {
                        ...
                        vm.setObjectGlobal('_execResult', { status: 1, result });
                    }
                    vm.waitingForResponse = false;
                    return result;
                },
        ...
        vm.setObjectGlobal('_db', {
            ...
            _getMany: function (handleId, keys) {
                const sync = vmSync();
                that._dbInstances[handleId].getMany(keys, function (err, val) {
                    if (err) {
                        sync.fails(false);
                    } else {
                        sync.return(val);
            ...
            _batch: function (handleId, items) {
                let sync = vmSync();
                that._dbInstances[handleId].batch(items, function (err) {
                    sync.return(err);
                });
                return true;
            }
        ...
        vm.injectScript('new ' + function () {
            ...
            global.KeyValue = function (dbName) {
                ...
                this.getMany = function (keys) {
                    _db._getMany(that.handler, keys);
                    return waitForReturn();
                };
        ...
        vm.setObjectGlobal('_events', {
            ...
            _emit: function (event) {
                let sync = vmSync();
                that.events.emitEvents(event, function (err) {
                    if (err) {
                        sync.return(false);
        ...
        vm.injectScript('new ' + function () {
            ...
            global.Events = {
                 ...
                 * @return {*}
                 */
                emit: function (eventName, eventType, events, emitType) {
                    let state = global.getState();

                    assert.true(Array.isArray(events), 'Event arguments must be an array');
                    assert.true(events.every(event => event.length <= 10), 'Event can take 10 arguments maximum');
                    assert.true(typeof eventName === 'string', 'Event name must be a string');

                    const event = {
                        emitType,
                        name: eventName,
                        type: eventType,
                        blockIndex: state.block.index,
                        timestamp: state.block.timestamp,
        ...
        vm.setObjectGlobal('_nftStorages', {
            /**
             *
             * @param {string} event
             * @param {array}  args
            ...
            _push: function (event) {
                const sync = vmSync();
                return that.events.emitEvents(event, (err) => {
                    if (err) {
                        sync.return(false);
                        throw 'Event handling error ' + err;
                    }
            ...
            _transferNft: function (event) {
                const sync = vmSync();
                return that.events.transferNft(event, (err) => {
                    if (err) {
                        sync.fails(err);
                        throw err;
                    }

                    return sync.return(true);
                })
            },
            _transferNftByValidationId: function (event) {
                const sync = vmSync();
                return that.events.transferNftByValidationId(event, (err) => {
                    if (err) {
                        sync.fails(err);
                        throw err;
                    }
            ...
            _lockOrUnlockNftByValidationId: function (amount, validationId, owner, isFreeze) {
                const sync = vmSync();
                return that.events.lockOrUnlockNftByValidationId(amount, validationId, owner, isFreeze, (err, nonces) => {
                    if (err) {
                        sync.fails(err);
                        throw err;
                    }
                    return sync.return(JSON.stringify(nonces));
                })
            },
        });
        ...
        vm.injectScript('new ' + function () {
            let waitForReturn = global.waitForReturn;
            let _nftStorages = global._nftStorages;
            global._nftStorages = undefined;
            global.NftStorages = {
                /**
                 * Emits event
                 * @param {string} event
                 ...
                 * @return {*}
                 */
                push: function (eventName, eventType, nfts) {
                    const state = global.getState();

                    assert.true(Array.isArray(nfts), 'Nfts arguments must be an array');
                    assert.true(nfts.every(nft => nft.length <= 10), 'Event can take 10 arguments maximum');
                    assert.true(typeof eventName === 'string', 'Event name must be a string');
                    ...
                    _nftStorages._push(event);
                    return waitForReturn();
                },
                transferNft: function (eventName, eventType, from, to, amount) {
                    const state = global.getState();
                    ...
                    _nftStorages._transferNft(event);
                    return waitForReturn();
                },
                transferNftByValidationId: function (eventName, eventType, validationId, isFreeze, from, to, amount) {
                    const state = global.getState();
                    ...
                    const event = {
                        ...
                        timestamp: state.block.timestamp,
                        contractAddress: state.contractAddress,
                        params: [validationId, isFreeze, from, to, amount],
                    };

                    _nftStorages._transferNftByValidationId(event);
                    return waitForReturn();
                },
                freezeNftByValidationId: function (amount, validationId, owner) {
                    _nftStorages._lockOrUnlockNftByValidationId(amount, validationId, owner, true);
                    return waitForReturn();
                },
                unFreezeNftByValidationId: function (amount, validationId, owner) {
                    _nftStorages._lockOrUnlockNftByValidationId(amount, validationId, owner, false);
                    return waitForReturn();
                },
            };
        });
        ...
        vm.injectSource(__dirname + '/internalModules/NFTTokenRegister.js');
        vm.injectSource(__dirname + '/internalModules/Contract.js');
        vm.injectSource(__dirname + '/internalModules/TokenContract.js');
        vm.injectSource(__dirname + '/internalModules/TokenNFTContract.js');
        vm.injectSource(__dirname + '/internalModules/Event.js');
        vm.injectSource(__dirname + '/internalModules/NftStorage.js');
     ...
     * @param cb
     */
    async rollbackAndClearContractsChain(state, cb) {
        for (const instance of this._instanceCallstack) {
            await (() => new Promise(resolve => {
                    const contractAddress = instance.vm.state.contractAddress;
                    const blockIndex = state.block.index;
                    const db = instance.db;

                    this.events.rollback(contractAddress, blockIndex, () => {
                        db.rollback(() => {
                            resolve();
                        })
                    })
     ...
     * @param cb
     */
    async deployAndClearContractsChain(state, cb) {
        for (const instance of this._instanceCallstack) {
            await (() => new Promise(resolve => {
                    const contractAddress = instance.vm.state.contractAddress;
                    const blockIndex = state.block.index;
                    ...
                    this.events.deploy(contractAddress, blockIndex, () => {
                        db.deploy(() => {
                            resolve();
                        })
                    })
    ...
    deployContractMethodIsCall = false;
    deployContractMethodCalls = [];

    async _deployContractMethodChecker() {
        storj.put('isContractMethodDeploy', this.deployContractMethodCalls.length > 0);
        
        if (this.deployContractMethodCalls.length === 0) return;
        if (this.deployContractMethodIsCall) return;

        this.deployContractMethodIsCall = true;

        const { address, method, args, state, cb, accountName } = this.deployContractMethodCalls.shift();
        logger.info(`Call deploy contact ${address} metod ${method}`);
        
        this._deployContractMethod(address, method, args, state, (err, result) => { 
            this.deployContractMethodIsCall = false; 
            logger.info(`Call deploy contact ${address} metod ${method}. Done!`);
            this._deployContractMethodChecker();
            cb(err, result);
        }, accountName);
    }

    deployContractMethod(address, method, args = [], state = {}, cb, accountName = false) {
        this.deployContractMethodCalls.push({ address, method, args, state, cb, accountName });
        this._deployContractMethodChecker();
    }
     ...
     */
    _deployContractMethod(address, method, args = [], state = {}, cb, accountName = false) {
        let that = this;
        ...
        that.getContractLimits(address, async function (limits) {            
            ...
            that.blockchain.generateNextBlockAuto(callBlock, function (generatedBlock) {
                console.time('_handleBlockReplay');
                that.events._handleBlockReplay(generatedBlock.index, function (err) {
                    console.timeEnd('_handleBlockReplay');
                    ...
                    const blockData = JSON.parse(generatedBlock.data);
                    that.events.createBlockHeaderData(blockData, generatedBlock, (err) => {
                        if (err) {
                            cb(err);
                            return;
                        }

                        console.time('_handleBlock');

                        that._handleBlock(blockData, generatedBlock, false, (err) => {
                            console.timeEnd('_handleBlock');
                            ...
                            that.events._transactions[String(generatedBlock.index)] = {};
    ...
    getOrCreateContractInstance(address, code, state, cb) {
        ...
        let destroyInstanceTimer = function (instance) {
            ...
            if(
                that._instanceCallstack.indexOf(instance) !== -1 || 
                instance.vm.isBusy() || 
                that.ecmaCalls.length > 0 ||
                that.deployContractMethodCalls.length > 0
            ) {
    ...
    _registerRPCMethods() {
        ...
        app.use((req, res, next) => {
            ...
            try {
                ...
                const publicKey = headers['e.public'];
                const timestamp = Number(headers['e.timestamp']);
                const sign = headers['e.sign'];
                const signText = `${publicKey}${timestamp}`;
                const isSignCurrect = this.cryptography.verify(signText, sign, publicKey);

                if (isSignCurrect) {
                    const dateFromTimestamp = new Date(timestamp);
                    const isTimestampCurrect = dateFromTimestamp.getTime() > new Date().getTime();

                    if (isTimestampCurrect) {
                        req.publicKey = publicKey;
                    }
                }
            } catch (e) {
                if (e.message != 'Invalid address') {
                    console.log('REST Auth:', 'Error:', e);
                }
            }
        ...
        app.get('/contracts/ecma/getInfo/', async (req, res) => {
            res.send({
                isReady: this.ready,
                latestBlock: this._lastKnownBlock,
            })
        })
        
        app.get('/contracts/ecma/getContractInfo/:address/', async (req, res) => {
            const address = req.params.address;
            try {
                const instance = await this.getContractInstanceByAddressAsync(address);
                res.send({ 
                    info: instance.info,
                    initState: instance.vm.state,
                    source: instance.vm.script,
                })
            } catch (e) {
                res.send({ error: true });
            }
        ...
        app.get('/contracts/ecma/contractExists/:address/', async (req, res) => {
            const address = req.params.address;
            try {
                const isContractExist = await this.contractExists(address);
                res.send({ isContractExist });
            } catch (e) {
                res.send({ error: true });
            }
        })

        app.get('/contracts/ecma/getContractProperty/:address/:property/', async (req, res) => {
            const address = req.params.address;
            const property = req.params.property;
            try {
                const caller = req.publicKey;
                const contract = new ContractConnector(this, address, false, caller);
                const contractProperty = await contract.getPropertyPromise(property);
                res.send({ result: contractProperty });
            } catch (e) {
                res.send({ error: true });
            }
        });

        app.post('/contracts/ecma/callMethod/:address/:method/', async (req, res) => {
            try {
                ...
                const accountName = false;
                const caller = req.publicKey;
                const contract = new ContractConnector(this, address, accountName, caller);
                const args = req.body.args;
                contract.registerMethod(method);
        ...
        app.post('/contracts/ecma/deployMethod/:address/:method/', async (req, res) => {
            try {
                if(typeof req.body.argsEncoded !== 'undefined') {
                    req.body.args = JSON.parse(req.body.argsEncoded);
                } else {
                ...
                const contract = new ContractConnector(this, address, accountName, caller);
                const args = req.body.args;
                contract.registerDeployMethod(method);
                let result = await contract[method](...args);
                try {
                    result = JSON.parse(result);
                } catch {}
