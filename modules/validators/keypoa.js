...
function isValidNewBlock(newBlock, previousBlock) {
    ...
    if (previousBlock.index >= 0 && previousBlock.index <= 2 && newBlock.index - 1 === previousBlock.index) {
        return true;
    }
...
function generateNextBlock(blockData, cb, cancelCondition, timestamp) {
    ...
    blockchain.getLatestBlock(function (previousBlock) {
        if(!previousBlock) {
            logger.info(`Previos block not found`);
            return cb(null);
        }
...
function generateEmptyBlockCheck() {
    if(blockchain !== null && generateEmptyBlocks) {
        ...
        if(!isKeyFromKeyStorage(blockchain.wallet.keysPair.public)) {
            logger.info("Info: We can't generate empty KeyPoA blocks");
            generateEmptyBlocks = false;
            return false;
        }

        if (storj.get('isContractMethodDeploy')) {
            logger.info("Info: Can't generate empty KeyPoA blocks, contract method is deploy");
            return false;
        }
        ...
        blockchain.getLatestBlock(function (previousBlock) {
            ...
            if(moment().utc().valueOf() - previousBlock.timestamp > blockchain.config.generateEmptyBlockDelay) {
                logger.info("Info: Create empty block");
                generateEmptyBlock();
            }
        });
    }
...
function handleKeyBlock(blockData, block, cb) {
    if(!isValidBlockAdminSign(block) && block.index > 1) {
